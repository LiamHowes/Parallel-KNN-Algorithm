...
#define num_cols 12 // doubled dimensionality of 4D
...
//regression
    if(choice==0){
        double reg_data[num_rows][num_cols]; //12D array for higher dimensional tests
        ...
        double reg_query[num_cols-1]; // query has 11 values
        ...
        std::cerr<<"estimated 12th column value = "<<answer<<std::endl;
...
//classification
        ...
        double clas_data[num_rows][num_cols]; //12D array for higher dimensional tests
        ...
        double clas_query[num_cols-1]; // query has 11 values
        for(int i=0; i<num_cols-1; i++){

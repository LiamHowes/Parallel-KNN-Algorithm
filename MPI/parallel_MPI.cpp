...
#include <string.h> // getting data from file
...
const int k = 3; // define our k value;
#define num_cols 12 // "Dimensionality"
#define num_rows 10000 // size of our data array
const int choice = 1; // 0 = regression, 1 = classification
const int thread_count = 8; // need to change this based on how many processes you want to run(before building and running)
...
void KNN(double data[split][num_cols + 1], double query[num_cols - 1], double  nearest_neighbours[k][num_cols + 1]){
    for (int i = 0; i < split; i++) { //for every row ...
        auto value = new double[num_cols - 1];
        for (int j = 0; j < num_cols - 1; j++) {//and column in search data
        // store the num_cols-1 variables to cross-reference query with
            value[j] = data[i][j];
        }
...
int main() {
    ...
    //regression
    if (choice == 0) {
        // allocate memory using "auto" (C++11)
        double reg_data[num_rows][num_cols+1]; //4D array for higher dimensional tests
        double reg_query[num_cols - 1]; // query has 3 values
        ...
        start = clock();
        {
            ...
            int thread = my_rank;
            auto data_partition = new double[split][num_cols + 1];
            // can use MPI_Scatter to split the original data set into partitions for each process
            MPI_Scatter(reg_data, split * (num_cols+1), MPI_DOUBLE, data_partition, split * (num_cols+1), MPI_DOUBLE, 0, MPI_COMM_WORLD);
            // each thread gets a partition of the full data
            auto nearest_neighbours = new double[k][num_cols + 1]; //+1 to store the distance value for each training data (stored in last column)
            KNN(data_partition, reg_query, nearest_neighbours);
            // store each threads nearest neighbours in a combined, final array
            // can use MPI_Gather to then combine all the split arrays into final_sorted_data array
            MPI_Gather(nearest_neighbours, (k * (num_cols + 1)), MPI_DOUBLE, final_sorted_data, (k * (num_cols + 1)), MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }
        if (my_rank == 0) {
            // sort the combined, master array
            for (int i = 0; i < (k * thread_count); i++) {
            ...
            std::cerr << "estimated missing column value = " << answer << std::endl;
    ...
    //classification
        ...
        double clas_data[num_rows][num_cols]; //4D array for higher dimensional tests
        double clas_query[num_cols - 1]; // query has 3 values
        double final_sorted_data[k * thread_count][num_cols + 1]; //+1 to store the distance value for each training data (stored in last column)
        ...
        start = clock();
        {
            ...
            // can use MPI_Scatter to split the original data set into partitions for each process
            MPI_Scatter(clas_data, split * (num_cols + 1), MPI_DOUBLE, data_partition, split * (num_cols + 1), MPI_DOUBLE, 0, MPI_COMM_WORLD);
            // each thread gets a partition of the full data
            double nearest_neighbours[k][num_cols + 1]; //+1 to store the distance value for each training data (stored in last column)
            KNN(data_partition, clas_query, nearest_neighbours);
            // store each threads nearest neighbours in a combined, final array
            // can use MPI_Gather to then combine all the split arrays into final_sorted_data array
            MPI_Gather(nearest_neighbours, (k* (num_cols + 1)), MPI_DOUBLE, final_sorted_data, (k* (num_cols + 1)), MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }
